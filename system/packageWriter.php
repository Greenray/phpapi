...
class packageWriter extends htmlWriter {
        ...
        foreach ($packages as $packageName => $package) {
            ...
            if ($interfaces) {
                ...
                foreach ($interfaces as $name => $interface) {
                    $output[$name]['path'] = str_repeat('../', $this->depth).$interface->path();
            ...
            if ($traits) {
                ...
                foreach ($traits as $name => $trait) {
                    $output[$name]['path'] = str_repeat('../', $this->depth).$trait->path();
            ...
            if ($classes) {
                ...
                foreach ($classes as $name => $class) {
                    $output[$name]['path'] = str_repeat('../', $this->depth).$class->path();
            ...
            if ($functions) {
                ...
                foreach ($functions as $name => $function) {
                    $output[$name]['path'] = str_repeat('../', $this->depth).$function->path();
            ...
            if ($globals) {
                ...
                foreach ($globals as $name => $global) {
                    $output[$name]['path'] = str_repeat('../', $this->depth).$global->path();
            ...
            if ($exceptions) {
                ...
                foreach ($exceptions as $name => $exception) {
                    $output[$name]['path'] = str_repeat('../', $this->depth).$exception->path();
            ...
            $this->tree($package, $package->path().DS.'package-tree', $package->name);
            ...
            $this->write($package->path().DS.'package-summary.html', $package->name);
        }
        $this->sections[0] = ['title' => 'Overview',   'url' => $index.'.html'];
     ...
     * @param  array  &$output  Reference to the result array
    ...
    private function displayTree($elements, &$tree, $parent = NULL) {
        ...
        foreach ($elements as $i => $element) {
            ...
            if ($element->superclass === $parent) {
                ...
                $tree[]['item'] = '<li>
                    <a href="'.str_repeat('../', $this->depth).$element->parent->path().DS.'package-summary.html">'.$element->parent->name.'</a> \
                    <a href="'.str_repeat('../', $this->depth).$element->path().'">'.$element->name.'</a>';
