...
class phpapi {
    ...
    public function execute(&$rootDoc) {
        ...
        require SYSTEM.'htmlWriter.php';
        require SYSTEM.'overviewSummaryWriter.php';
        require SYSTEM.'packageWriter.php';
        require SYSTEM.'classWriter.php';
        require SYSTEM.'functionWriter.php';
        require SYSTEM.'globalWriter.php';
        require SYSTEM.'indexWriter.php';
        require SYSTEM.'deprecatedWriter.php';
        require SYSTEM.'todoWriter.php';

        $docletFile = SYSTEM.$this->doclet.DS.$this->doclet.'.php';
        if (is_file($docletFile)) {
            ...
            $this->verbose('Loading formatter '.$formatter);

            require_once SYSTEM.$formatter.'.php';

            $doclet = &new $this->doclet($rootDoc, new $formatter);
    ...
    private function getFiles($files, $dir) {
        ...
        foreach ($files as $filename) {
            ...
            } elseif (empty($this->options['subDirs'])) $this->error('Cannot find file '.$filename);
        }
        #
        # Recurse into subdir
        #
...
//                                            $this->verbose('Found global variable: '.$global->name.' in package '.$const->package);
     ...
     * @param  array  $rootDoc Reference to the root of the parsed tokens
